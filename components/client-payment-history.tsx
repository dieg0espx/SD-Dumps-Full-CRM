...
export function ClientPaymentHistory({ payments }: ClientPaymentHistoryProps) {
    ...
    <div className="space-y-4 sm:space-y-6 px-1 sm:px-0">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 lg:gap-6">
        <Card className="shadow-sm hover:shadow-md transition-shadow">
          ...
          <CardContent className="px-3 sm:px-6">
            <div className="text-lg sm:text-xl lg:text-2xl font-bold">{formatCurrency(totalSpent)}</div>
      ...
      <Card className="shadow-sm">
        <CardHeader className="px-3 sm:px-6">
          <CardTitle className="text-base sm:text-lg lg:text-xl">Payment History</CardTitle>
        </CardHeader>
        <CardContent className="px-3 sm:px-6">
          {payments.length === 0 ? (
            <div className="text-center py-8 sm:py-12">
              <p className="text-gray-500 mb-2 text-sm sm:text-base">No payments found</p>
              <p className="text-xs sm:text-sm text-gray-400">Your payment history will appear here</p>
            </div>
          ) : (
                    ...
                    <div className="flex flex-col space-y-1 sm:space-y-2">
                      ...
                      <p className="text-xs sm:text-sm text-gray-600">
                        {payment.bookings?.start_date && payment.bookings?.end_date && (
                          <>
                            {format(new Date(payment.bookings.start_date), "MMM d")} -{" "}
                            {format(new Date(payment.bookings.end_date), "MMM d, yyyy")}
                          </>
                        )}
                      </p>
